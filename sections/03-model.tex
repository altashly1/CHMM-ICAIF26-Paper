For ticker $i$ and trading day $j$, with $P_{i,j}$ the session volume-weighted average price (VWAP), $\Delta t = 1/252$ the annualised trading-day step, and $r_f$ the risk-free rate (set to zero throughout, so the quantity below is the annualised log growth rate rather than an empirically risk-free-adjusted excess return), the annualised growth rate is
\begin{equation}
